...
const handler = async (m, { conn, command, args, text, usedPrefix }) => {
  ...
  if (command === 'الصوت') {
    try {
      ...
      if (size >= limit_a2) {
        await conn.sendMessage(m.chat, { text: `${resultados.descargar}` }, { quoted: m });
        return;
      }
      ...
      try {
        ...
        if (size >= limit_a2) {
          await conn.sendMessage(m.chat, { text: `${audio}` }, { quoted: m });
          return;
        }
        if (size >= limit_a1 && size <= limit_a2) {
          await conn.sendMessage(m.chat, { document: buff_aud, mimetype: 'audio/mpeg', fileName: ttl + `.mp3` }, { quoted: m });
  ...
  if (command === 'الفيديو') {
    try {
      ...
      try {
        ...
        if (size2 >= limit2) {
          await conn.sendMessage(m.chat, { text: `${video}` }, { quoted: m });
          return;
        }
        if (size2 >= limit1 && size2 <= limit2) {
          await conn.sendMessage(m.chat, { document: buff_vid, mimetype: 'video/mp4', fileName: ttl2 + `.mp4` }, { quoted: m });
...
handler.command = /^(الصوت|الفيديو)$/i;

...
let handler = async (m, { conn, text, command, usedPrefix }) => {
    ...
    if (!lister.includes(feature)) {
        return conn.sendButton(m.chat, cap, '𝑍𝛩𝐹𝐴𝑁•𝐵𝛩𝑇', pp, [['دولـة',`${usedPrefix + command} دولة`]], null, null, m);
    }
...
return await conn.relayMessage(m.chat, msg.message, { messageId: msg.key.id });
            ...
            console.log(error);
            return conn.sendButton(m.chat, `╮────────────────────────╭ـ\n│ حدث خطأ أثناء جلب البيانات. حاول مرة أخرى لاحقًا.\n╯────────────────────────╰ـ `, '𝑍𝛩𝐹𝐴𝑁•𝐵𝛩𝑇', pp, [['حاول مجددا',`${usedPrefix + command} أرقام ${additionalLink}`]], null, null, m);
        }

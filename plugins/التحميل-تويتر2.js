...
if (!text) throw `[❗] يرجي إدخال رابط أولا\nمثال: *${usedPrefix + command}* https://twitter.com/auronplay/status/1586487664274206720?s=20&t=3snvkvwGUIez5iWYQAehpw`;
if (enviando) return;
    enviando = true;
try {
 ...
 if (res?.result.type == 'video') {
     const caption = res?.result.caption ? res.result.caption : 'لا يوجد وصف';
    ...
    enviando = false;
    throw 'حدث خطأ أثناء معالجة الطلب. يرجى المحاولة لاحقاً.';
    return;
  }
};    
handler.command = /^((xs|xdl|تويتي)$/i;
...
const TwitterDL = async (url) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (!id)
      return resolve({
        status: "error",
        message: 'لم يتم العثور على معرف التويت.',
      });
      ...
      if (response.data.code !== 200) {
        return resolve({
          status: "error",
          message: 'تعذر الحصول على البيانات من تويتر. يرجى المحاولة لاحقاً.',
        });
      }
